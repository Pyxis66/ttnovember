# coding=utf-8
"""
This module represents TTNovember's plugin subsystem. This includes management and helper methods as well as the
...
def plugin_manager(init=False, plugin_folders=None, plugin_types=None, plugin_entry_points=None, plugin_disabled_list=None,
	...
	Arguments:
	    ...
	    plugin_folders (list): A list of folders (as strings containing the absolute path to them) in which to look for
	        ...
	        ``src/plugins`` within TTNovember's code base.
	    plugin_types (list): A list of recognized plugin types for which to look for provided implementations. If not
	        ...
	        :class:`~octoprint.plugin.TTNovemberPlugin`.
...
def plugin_settings_for_settings_plugin(plugin_key, instance, settings=None):
	...
	Arguments:
	    ...
	    settings (octoprint.settings.Settings): The settings instance to use. Defaults to the global TTNovember settings.
...
class PluginSettings(object):
	...
	@deprecated("PluginSettings.get_plugin_data_folder has been replaced by TTNovemberPlugin.get_plugin_data_folder",
	            includedoc="Replaced by :func:`~octoprint.plugin.types.TTNovemberPlugin.get_plugin_data_folder`",
	            since="1.2.0")
