# coding=utf-8
"""
This module bundles all of TTNovember's supported plugin implementation types as well as their common parent
class, :class:`TTNovemberPlugin`.
...
__copyright__ = "Copyright (C) 2014 The TTNovember Project - Released under terms of the AGPLv3 License"
...
class TTNovemberPlugin(Plugin):
	"""
	The parent class of all TTNovember plugin mixins.
...
class StartupPlugin(TTNovemberPlugin, SortablePlugin):
	"""
	The ``StartupPlugin`` allows hooking into the startup of TTNovember. It can be used to start up additional services
...
class ShutdownPlugin(TTNovemberPlugin, SortablePlugin):
	"""
	The ``ShutdownPlugin`` allows hooking into the shutdown of TTNovember. It's usually used in conjunction with the
	...
	def on_shutdown(self):
		"""
		Called upon the imminent shutdown of TTNovember.
...
class AssetPlugin(TTNovemberPlugin, RestartNeedingPlugin):
	"""
	...
	def get_assets(self):
		...
		The assets will be made available by TTNovember under the URL ``/plugin/<plugin identifier>/static/<path>``, with
		``plugin identifier`` being the plugin's identifier and ``path`` being the path as defined in the asset dictionary.

		Assets of the types ``js``, ``css`` and ``less`` will be automatically bundled by TTNovember using
...
class TemplatePlugin(TTNovemberPlugin, ReloadNeedingPlugin):
	"""
	Using the ``TemplatePlugin`` mixin plugins may inject their own components into the TTNovember web interface.

	Currently TTNovember supports the following types of injections out of the box:
	...
	About
	   Plugins may define additional panels into TTNovember's "About" dialog. Note that with the current implementation
	...
	the "helloworld" plugin in TTNovember's :ref:`Plugin Tutorial <sec-plugins-gettingstarted>`.
		   ...
		   .. list-table::
		      ...
		      * - mandatory
		        - Whether the wizard step is mandatory (True) or not (False). Optional,
		          defaults to False. If set to True, TTNovember will sort visually mark
...
class UiPlugin(TTNovemberPlugin, SortablePlugin):
	"""
	The ``UiPlugin`` mixin allows plugins to completely replace the UI served
	by TTNovember when requesting the main page hosted at `/`.

	TTNovember will query whether your mixin implementation will handle a
	...
	parameter. If you plugin returns `True` here, TTNovember will next call
	...
	used by the default TTNovember web interface. For more information see below.
	...
	**Example**
	...
	.. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/dummy_mobile_ui/__init__.py
	   :linenos:
	   :tab-width: 4
	   :caption: `dummy_mobile_ui/__init__.py <https://github.com/TTNovember/Plugin-Examples/blob/master/dummy_mobile_ui/__init__.py>`_

	.. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/dummy_mobile_ui/templates/dummy_mobile_ui_index.jinja2
	   :linenos:
	   :tab-width: 4
	   :caption: `dummy_mobile_ui/templates/dummy_mobile_ui_index.jinja2 <https://github.com/TTNovember/Plugin-Examples/blob/master/dummy_mobile_ui/templates/dummy_mobile_ui_index.jinja2>`_

	Try installing the above plugin ``dummy_mobile_ui`` (also available in the
	`plugin examples repository <https://github.com/TTNovember/Plugin-Examples/blob/master/dummy_mobile_ui>`_)
	into your TTNovember instance. If you access it from a regular desktop browser,
	...
	**Preemptive and Runtime Caching**
	...
	TTNovember will also cache your custom UI for you in its server side UI cache, making sure
	...
	def will_handle_ui(self, request):
		"""
		Called by TTNovember to determine if the mixin implementation will be
	...
	def on_ui_render(self, now, request, render_kwargs):
		"""
		Called by TTNovember to retrieve the response to send to the client
		...
		.. list-table::
		   ...
		   * - version
		     - TTNovember's version information. This is a ``dict`` with the
		       following keys:
		       ...
		       .. list-table::
		          ...
		          * - branch
		            - The GIT branch from which the TTNovember instance was built
		              (e.g. ``master``)
		          * - display
	...
	def get_ui_additional_key_data_for_cache(self):
		...
		Returns:
		    list, tuple: A list or tuple of strings to use in the cache key. Will be joined by TTNovember
	...
	def get_ui_additional_tracked_files(self):
		"""
		Allows to return additional files to track for validating existing caches. By default TTNovember
	...
	def get_ui_custom_etag(self):
		...
		TTNovember's version, current ``UI_API_KEY``, tracked file paths and ``LastModified`` value).
	...
	def get_ui_preemptive_caching_additional_unless(self):
		...
		TTNovember will call this method when processing a UI request, to determine whether to record the
	...
	def get_ui_custom_template_filter(self, default_template_filter):
		...
		Only relevant for UiPlugins that actually utilize the stock templates of TTNovember.
...
class WizardPlugin(TTNovemberPlugin, ReloadNeedingPlugin):
	"""
	The ``WizardPlugin`` mixin allows plugins to report to TTNovember whether
	...
	TTNovember will only display such wizard dialogs to the user which belong
	to plugins that
	...
	TTNovember will then display the wizard dialog provided by the plugin through
	the :class:`TemplatePlugin` mixin. Once the user finishes the wizard on the
	frontend, TTNovember will store that it already showed the wizard of ``myplugin``
	...
	have TTNovember reshow the dialog. E.g.
	...
	def is_wizard_required(self):
		...
		TTNovember will only include those wizards from plugins which are reporting
		their wizards as being required through this method returning ``True``.
		Still, if TTNovember already displayed that wizard in the same version
	...
	def get_wizard_version(self):
		"""
		The version of this plugin's wizard. TTNovember will only display a wizard
		of the same plugin and wizard version once to the user. After they
		finish the wizard, TTNovember will remember that it already showed this
		...
		way to notify TTNovember of these changes.
	...
	def get_wizard_details(self):
		"""
		Called by TTNovember when the wizard wrapper dialog is shown. Allows the plugin to return data
	...
	def on_wizard_finish(self, handled):
		"""
		Called by TTNovember whenever the user finishes a wizard session.
...
class SimpleApiPlugin(TTNovemberPlugin):
	"""
	...
	The simple API constructed by TTNovember for you will be made available under ``/api/plugin/<plugin identifier>/``.
	TTNovember will do some preliminary request validation for your defined commands, making sure the request body is in
	...
	def on_api_command(self, command, data):
		"""
		Called by TTNovember upon a POST request to ``/api/plugin/<plugin identifier>``. ``command`` will contain one of
		...
		If your plugin returns nothing here, TTNovember will return an empty response with return code ``204 No content``
		...
		:return: ``None`` in which case TTNovember will generate a ``204 No content`` response with empty body, or optionally
	...
	def on_api_get(self, request):
		"""
		Called by TTNovember upon a GET request to ``/api/plugin/<plugin identifier>``. ``request`` will contain the
...
class BlueprintPlugin(TTNovemberPlugin, RestartNeedingPlugin):
	"""
	...
	Your blueprint will be published by TTNovember under the base URL ``/plugin/<plugin identifier>/``, so the above
...
class SettingsPlugin(TTNovemberPlugin):
	"""
	Including the ``SettingsPlugin`` mixin allows plugins to store and retrieve their own settings within TTNovember's
	configuration.
	...
	.. warning::
	   ...
	   should have access to via :meth:`~octoprint.plugin.SettingsPlugin.get_settings_restricted_paths`. TTNovember will
	...
	def on_settings_save(self, data):
		...
		additional settings properties that are not stored within TTNovember's configuration.
	...
	def get_settings_version(self):
		...
		Use this to have TTNovember trigger your migration function if it detects an outdated settings version in
		config.yaml.
	...
	def on_settings_migrate(self, target, current):
		"""
		Called by TTNovember if it detects that the installed version of the plugin necessitates a higher settings version
		...
		Your plugin's implementation should take care of migrating any data by utilizing self._settings. TTNovember
	...
	def on_settings_cleanup(self):
		...
		version of TTNovember that still duplicated default data).
		"""
		import octoprint.util
...
class EventHandlerPlugin(TTNovemberPlugin):
	"""
	The ``EventHandlerPlugin`` mixin allows TTNovember plugins to react to any of :ref:`TTNovember's events <sec-events>`.
	TTNovember will call the :func:`on_event` method for any event fired on its internal event bus, supplying the
	event type and the associated payload. Please note that until your plugin returns from that method, further event
	processing within TTNovember will block - the event queue itself is run asynchronously from the rest of TTNovember,
	...
	def on_event(self, event, payload):
		"""
		Called by TTNovember upon processing of a fired event on the platform.
...
class SlicerPlugin(TTNovemberPlugin):
	"""
	Via the ``SlicerPlugin`` mixin plugins can add support for slicing engines to be used by TTNovember.

	"""

	def is_slicer_configured(self):
		"""
		Unless the return value of this method is ``True``, TTNovember will not register the slicer within the slicing
	...
	def get_slicer_properties(self):
		...
		same_device
		    True if the slicer runs on the same device as TTNovember, False otherwise. Slicers running on the same
		...
		progress_report
		    ``True`` if the slicer can report back slicing progress to TTNovember ``False`` otherwise.
		source_file_types
	...
	def do_slice(self, model_path, printer_profile, machinecode_path=None, profile_path=None, position=None, on_progress=None, on_progress_args=None, on_progress_kwargs=None):
		"""
		Called by TTNovember to slice ``model_path`` for the indicated ``printer_profile``. If the ``machinecode_path`` is ``None``,
		slicer implementations should generate it from the provided ``model_path``.

		If provided, the ``profile_path`` is guaranteed by TTNovember to be a serialized slicing profile created through the slicing
		...
		Please note that both ``on_progress_args`` and ``on_progress_kwargs`` as supplied by TTNovember might be ``None``,
...
class ProgressPlugin(TTNovemberPlugin):
	"""
	...
	def on_print_progress(self, storage, path, progress):
		"""
		Called by TTNovember on minimally 1% increments during a running print job.
	...
	def on_slicing_progress(self, slicer, source_location, source_path, destination_location, destination_path, progress):
		"""
		Called by TTNovember on minimally 1% increments during a running slicing job.
...
class AppPlugin(TTNovemberPlugin):
	"""
