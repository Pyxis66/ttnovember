...
def getSettings():
	...
	data = {
		...
		"filamentProfile": {
			"profiles": s.get(["filamentProfile", "profiles"]),
			"history" : s.get(["filamentHistory", "history"]),
		},
		#"filamentProfile": s.get(["filamentProfile"]), //profile 단일
		#"filamentProfile":{
		#	"filamentProfiles": s.get(["filamentProfile", "filamentProfiles"]),
		#},
		"system": {
...
def _get_plugin_settings():
	...
	for plugin in octoprint.plugin.plugin_manager().get_implementations(octoprint.plugin.SettingsPlugin):
		try:
			...
			logger.warn("in a way which has issues due to TTNovember's dynamic reloading after plugin operations.")
...
def _saveSettings(data):
	...
	#if "filamentProfile" in data.keys():
	#	if "filamentProfiles" in data["filamentProfile"]: s.set(["filamentProfile", "filamentProfiles"], data["filamentProfile"]["filamentProfiles"])
	#if "filamentProfile" in data.keys(): // 성공 단일
	#	s.set(["filamentProfile"], data["filamentProfile"])
	if "filamentProfile" in data.keys():
		if "profiles" in data["filamentProfile"]: s.set(["filamentProfile", "profiles"], data["filamentProfile"]["profiles"])
		if "history" in data["filamentProfile"]: s.set(["filamentProfile", "history"], data["filamentProfile"]["history"])
