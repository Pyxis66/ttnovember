...
class TTNovemberFlaskRequest(flask.Request):
	...
	def cookie_suffix(self):
		...
		session and other cookies from one TTNovember instance on our machine with those of another
...
class TTNovemberFlaskResponse(flask.Response):
...
def restricted_access(func):
	...
	done for TTNovember's Access Control plus that any conditions of the
	...
	If TTNovember's Access Control has not been setup yet (indicated by the "firstRun"
	...
	def decorated_view(*args, **kwargs):
		# if TTNovember hasn't been set up yet, abort
		if settings().getBoolean(["server", "firstRun"]) and settings().getBoolean(["accessControl", "enabled"]) and (octoprint.server.userManager is None or not octoprint.server.userManager.hasBeenCustomized()):
			return flask.make_response("TTNovember isn't setup yet", 403)
...
def collect_core_assets(enable_gcodeviewer=True, preferred_stylesheet="css"):
	...
	assets["js"] = [
		...
		'js/app/viewmodels/commandcenter.js',
