...
class Server(object):
	...
	def run(self):
		...
		def octoprint_plugin_inject_factory(name, implementation):
			"""Factory for injections for all TTNovemberPlugins"""
			if not isinstance(implementation, octoprint.plugin.TTNovemberPlugin):
				return None
			props = dict()
		...
		transforms = [util.tornado.GlobalHeaderTransform.for_headers("TTNovemberGlobalHeaderTransform",
		                                                             headers=headers,
		                                                             removed_headers=removed_headers)]
		...
		try:
			# this is the main loop - as long as tornado is running, TTNovember is running
			ioloop.start()
			self._logger.debug("Tornado's IOLoop stopped")
		except (KeyboardInterrupt, SystemExit):
			pass
		except:
			self._logger.fatal("Now that is embarrassing... Something really really went wrong here. Please report this including the stacktrace below in TTNovember's bugtracker. Thanks!")
	...
	def _check_for_root(self):
		if "geteuid" in dir(os) and os.geteuid() == 0:
			exit("You should not run TTNovember as root!")
	...
	def _setup_app(self, app):
		from octoprint.server.util.flask import ReverseProxiedEnvironment, TTNovemberFlaskRequest, TTNovemberFlaskResponse
		...
		TTNovemberFlaskRequest.environment_wrapper = reverse_proxied
		app.request_class = TTNovemberFlaskRequest
		app.response_class = TTNovemberFlaskResponse
	...
	def _setup_assets(self):
		...
		js_client = [
			...
			"js/app/client/commandcenter.js",
