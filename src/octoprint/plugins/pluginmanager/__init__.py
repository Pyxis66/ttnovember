...
class PluginManagerPlugin(octoprint.plugin.SimpleApiPlugin,
	...
	def command_install(self, url=None, path=None, force=False, reinstall=None, dependency_links=False):
		...
		try:
			...
			#
			#   Requirement already satisfied (use --upgrade to upgrade): TTNovember-Plugin==1.0 from \
			#     https://example.com/foobar.zip in <lib>
			#   Requirement already satisfied (use --upgrade to upgrade): TTNovember-Plugin in <lib>
			#   Requirement already satisfied (use --upgrade to upgrade): TTNovember-Plugin==1.0 from \
		...
		#
		#   Successfully installed TTNovember-Plugin-1.0 Dependency-One-0.1 Dependency-Two-9.3
		#
		# or this:
		#
		#   Successfully installed TTNovember-Plugin Dependency-One Dependency-Two
		#   Cleaning up...
		#
		...
		if new_plugin is None:
			self._logger.warn("The plugin was installed successfully, but couldn't be found afterwards to "
			                  "initialize properly during runtime. Please restart TTNovember.")
	...
	def _is_octoprint_compatible(self, octoprint_version, compatibility_entries):
		...
		for octo_compat in compatibility_entries:
			try:
				...
				s = next(pkg_resources.parse_requirements("TTNovember" + octo_compat))
				if octoprint_version in s:
					break
			except:
				self._logger.exception("Something is wrong with this compatibility string for TTNovember: {}".format(octo_compat))
		else:
			return False
	...
	def _get_octoprint_version(self, base=False):
		...
		# A leading v is common in github release tags and old setuptools doesn't remove it. While TTNovember's
...
__plugin_description__ = "Allows installing and managing TTNovember plugins"
