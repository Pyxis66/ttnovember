		...
		try:
			...
			try:
				...
				if "__version" in data:
					...
					self._logger.info("Can't determine version of TTNovember version cache was created for, not using it")
					return

				from octoprint._version import get_versions
				octoprint_version = get_versions()["version"]
				if data_version != octoprint_version:
					self._logger.info("Version cache was created for another version of TTNovember, not using it")
					return
	...
	def _populated_check(self, target, check):
		...
		if target == "octoprint":
			...
			if result["displayName"] is None:
				...
				result["displayName"] = to_unicode(gettext("TTNovember"), errors="replace")
...
__plugin_description__ = "Allows receiving update notifications and performing updates of TTNovember and plugins"
__plugin_disabling_discouraged__ = gettext("Without this plugin TTNovember will no longer be able to "
