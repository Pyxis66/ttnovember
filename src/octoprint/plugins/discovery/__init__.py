...
__plugin_description__ = "Makes the TTNovember instance discoverable via Bonjour/Avahi/Zeroconf and uPnP"
__plugin_disabling_discouraged__ = gettext("Without this plugin your TTNovember instance will no longer be "
...
class DiscoveryPlugin(octoprint.plugin.StartupPlugin,
	...
	##~~ StartupPlugin API -- used for registering TTNovember's Zeroconf and SSDP services upon application startup
	...
	##~~ ShutdownPlugin API -- used for unregistering TTNovember's Zeroconf and SSDP service upon application shutdown
	...
	def zeroconf_register(self, reg_type, name=None, port=None, txt_record=None):
		...
		:param name: displayable name of the service, if not given defaults to the TTNovember instance name
		:param port: port to register for the service, if not given defaults to TTNovember's (public) port
	...
	def zeroconf_unregister(self, reg_type, port=None):
		...
		:param port: the port of the service to be unregistered, defaults to TTNovember's (public) port if not given
		:return:
		"""
	...
	def _create_http_txt_record_dict(self):
		"""
		Creates a TXT record for the _http._tcp Zeroconf service supplied by this TTNovember instance.
	...
	def _create_octoprint_txt_record_dict(self):
		"""
		Creates a TXT record for the _octoprint._tcp Zeroconf service supplied by this TTNovember instance.
		...
		The following keys are defined:
		  ...
		  * `model`: Model of the device that is running TTNovember
		  * `vendor`: Vendor of the device that is running TTNovember
	...
	def _ssdp_register(self):
		"""
		Registers the TTNovember instance as basic service with a presentation URL pointing to the web interface
		"""
	...
	def _ssdp_unregister(self):
		"""
		Unregisters the TTNovember instance again
		"""
	...
	def get_instance_name(self):
		...
		if name:
			return u"TTNovember instance \"{}\"".format(name)
		else:
			import socket
			return u"TTNovember instance on {}".format(socket.gethostname())
