...
class PipCaller(CommandlineCaller):
	...
	def _setup_pip(self):
		...
		if pip_version in self.__class__.broken:
			self._logger.error("This version of pip is known to have bugs that make it incompatible with how it needs to be used by TTNovember. Please upgrade your pip version.")
			return

		# Now figure out if pip belongs to a virtual environment and if the
		# default installation directory is writable.
		#
		# The idea is the following: If TTNovember is installed globally,
	...
	def _check_pip_setup(self, pip_command):
		...
		with _cache_mutex:
			...
			# out for a generic pip command whether TTNovember can even install anything with that
