function DataUpdater(allViewModels, connectCallback, disconnectCallback) {
    ...
    self.connect = function() {
        ...
        TTNovember.socket.connect({debug: !!SOCKJS_DEBUG});
        ...
        self._ifInitialized(function() {
            ...
            if (type == "PrintCancelled") {
                ...
                if (payload.error && payload.error.indexOf("autodetect") == -1) { // ignore "failed to autodetect"
                    new PNotify({
                            title: gettext("Unhandled communication error"),
                            text: _.sprintf(gettext("The was an unhandled error while talking to the printer. Due to that TTNovember disconnected. Error: %(error)s"), payload),
                            type: "error",
                            hide: false
                    });
                }
    ...
    TTNovember.socket.onDisconnected = self._onDisconnected;
    TTNovember.socket.onReconnectAttempt = self._onReconnectAttempt;
    TTNovember.socket.onReconnectFailed = self._onReconnectFailed;
    TTNovember.socket.onRateTooHigh = self._onDecreaseRate;
    TTNovember.socket.onRateTooLow = self._onIncreaseRate;
    TTNovember.socket
