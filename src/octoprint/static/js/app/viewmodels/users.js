$(function() {
    function UsersViewModel(parameters) {
        ...
        self.addUser = function(user) {
            if (!user) {
                throw TTNovember.InvalidArgumentError("user must be set");
            }

            return TTNovember.users.add(user)
                .done(self.fromResponse);
        };
        ...
        self.removeUser = function(user) {
            ...
            return TTNovember.users.delete(user.name)
                .done(self.fromResponse);
        };
        ...
        self.updateUser = function(user) {
            ...
            return TTNovember.users.update(user.name, user.active, user.admin)
                .done(self.fromResponse);
        };

        self.updatePassword = function(username, password) {
            return TTNovember.users.changePassword(username, password);
        };

        self.generateApikey = function(username) {
            return TTNovember.users.generateApiKey(username);
        };

        self.deleteApikey = function(username) {
            return TTNovember.users.resetApiKey(username);
        };
