$(function() {
    function TemperatureViewModel(parameters) {
        ...
        self.updatePlot = function(force) {
            ...
            if (graph.length) {
                ...
                if (!self.plot || force) {
                    ...
                    if (!TTNovember.coreui.browser.mobile) {
        ...
        self.updateLegend = function(replaceLegendLabel) {
            ...
            if (pos && !TTNovember.coreui.browser.mobile) {
        ...
        self.setTarget = function(item, form) {
            ...
            if (value === undefined || (typeof(value) == "string" && value.trim() == "")) return TTNovemberClient.createRejectedDeferred();
        ...
        self.setTargetFromProfile = function(item, profile) {
            if (!profile) return TTNovemberClient.createRejectedDeferred();
        ...
        self.setTargetToValue = function(item, value) {
            ...
            try {
                ...
                return TTNovemberClient.createRejectedDeferred();
            }

            if (value < 0 || value > 999) return TTNovemberClient.createRejectedDeferred();
        ...
        self.setOffsetToValue = function(item, value) {
            ...
            if (value < -50 || value > 50) return TTNovemberClient.createRejectedDeferred();
        ...
        self._setToolTemperature = function(tool, temperature) {
            ...
            return TTNovember.printer.setToolTargetTemperatures(data);
        };

        self._setToolOffset = function(tool, offset) {
            var data = {};
            data[tool] = parseInt(offset);
            return TTNovember.printer.setToolTemperatureOffsets(data);
        };

        self._setBedTemperature = function(temperature) {
            return TTNovember.printer.setBedTargetTemperature(parseInt(temperature));
        };

        self._setBedOffset = function(offset) {
            return TTNovember.printer.setBedTemperatureOffset(parseInt(offset));
        };
        ...
        self.onStartup = function() {
            var graph = $("#temperature-graph");
            if (graph.length && !TTNovember.coreui.browser.mobile) {
