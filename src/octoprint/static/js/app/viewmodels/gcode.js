    ...
    function GcodeViewModel(parameters) {
        ...
        self.loadFile = function(path, date){
            ...
            if (self.status == "idle" && self.errorCount < 3) {
                self.status = "request";
                TTNovember.files.download("local", path)
        ...
        self._processData = function(data) {
            ...
            if(self.loadedFilepath
                ...
                if (TTNovember.coreui.browserTabVisible && self.tabActive && self.currentlyPrinting && self.renderer_syncProgress() && !self.waitForApproval()) {
                ...
                if (data.job.file.path && data.job.file.origin != "sdcard"
                    ...
                    if (data.job.file.size > CONFIG_GCODE_SIZE_THRESHOLD || (TTNovember.coreui.browser.mobile && data.job.file.size > CONFIG_GCODE_MOBILE_SIZE_THRESHOLD)) {
