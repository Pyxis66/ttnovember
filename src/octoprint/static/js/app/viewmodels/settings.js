$(function() {
    function SettingsViewModel(parameters) {
        ...
        self.filamentProfile_profiles = ko.observableArray(undefined);
        self.filamentProfile_history = ko.observableArray(undefined);
        ...
        self.addFilamentProfile = function() {
            self.filamentProfile_profiles.push({maker: "ThingnThinks", material: "PLA", color: "White", extruder:200, bed:60});
        };

        self.removeFilamentProfile = function(filamentProfile) {
            self.filamentProfile_profiles.remove(filamentProfile);
        };
        //history확인
        self.addFilamentHistory = function() {
            self.filamentProfile_history.push({maker: "ThingnThinks", material: "PLA", color: "White", extruder:200, bed:60});
        };

        self.removeFilamentHistory = function(filamentProfile) {
            self.filamentProfile_history.remove(filamentProfile);
        };
        ...
        self.testWebcamSnapshotUrl = function(viewModel, event) {
            ...
            TTNovember.util.testUrl(self.webcam_snapshotUrl(), {method: "GET", response: "bytes"})
        ...
        self.testWebcamFfmpegPath = function() {
            ...
            TTNovember.util.testExecutable(self.webcam_ffmpegPath())
                .done(function(response) {
                    if (!response.result) {
                        if (!response.exists) {
        ...
        self.onStartup = function() {
            ...
            self.translationUploadElement.fileupload({
                ...
                maxNumberOfFiles: 1,
                autoUpload: false,
                headers: TTNovember.getRequestHeaders(),
                add: function(e, data) {
        ...
        self.onAllBound = function(allViewModels) {
            ...
            self.settingsDialog.on('show', function(event) {
                TTNovember.coreui.settingsOpen = true;
            ...
            self.settingsDialog.on('hidden', function(event) {
                TTNovember.coreui.settingsOpen = false;
                if (event.target.id == "settings_dialog") {
        ...
        self.generateApiKey = function() {
            ...
            showConfirmationDialog(gettext("This will generate a new API Key. The old API Key will cease to function immediately."),
                function() {
                    TTNovember.settings.generateApiKey()
                        .done(function(response) {
        ...
        self.requestData = function(local) {
            ...
            // perform the request
            self.receiving(true);
            return TTNovember.settings.get()
                .always(function() {
        ...
        self.requestTranslationData = function() {
            return TTNovember.languages.list()
        ...
        self.deleteLanguagePack = function(locale, pack) {
            TTNovember.languages.delete(locale, pack)
                .done(self.fromTranslationResponse);
        };
        ...
        self.fromResponse = function(response, local) {
            ...
            var specialMappings = {
                ...
                filamentProfile: function(value) { self.filamentProfile($.extend(true, [], value)); }

                //filamentProfile: {
                //    filamentProfiles: function(value) { self.filamentProfile_filamentProfile($.extend(true, [], value)); }
                //}
            };
        ...
        self.saveData = function (data, successCallback, setAsSending) {
            ...
            self.active = true;
            return TTNovember.settings.save(data)
