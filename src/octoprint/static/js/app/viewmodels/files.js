$(function() {
    function FilesViewModel(parameters) {
        ...
        self.requestData = function(params) {
            ...
            return self._otherRequestInProgress = TTNovember.files.list(true, force)
        ...
        self.addFolder = function() {
            ...
            TTNovember.files.createFolder(location, name, self.currentPath())
                .done(function(data) {
                    self.requestData({
                        focus: {
        ...
        self.loadFile = function(data, printAfterLoad) {
            ...
            if (printAfterLoad && self.listHelper.isSelected(data) && self.enablePrint(data)) {
                ...
                TTNovember.files.select(data.origin, data.path, print);
            }
        };
        ...
        self.initSdCard = function() {
            TTNovember.printer.initSd();
        };

        self.releaseSdCard = function() {
            TTNovember.printer.releaseSd();
        };

        self.refreshSdFiles = function() {
            TTNovember.printer.refreshSd();
        };
        ...
        self._removeEntry = function(entry, event) {
            ...
            TTNovember.files.delete(entry.origin, entry.path)
                .done(function() {
                    self.requestData()
                        .done(function() {
                            deferred.resolve();
