...
class TTNovemberDevelCommands(click.MultiCommand):
	"""
	...
	def plugin_new(self):
		...
		def command(name, package, author, email, description, license, homepage, source, installurl, identifier):
			"""Creates a new plugin based on the TTNovember Plugin cookiecutter template."""
			...
			with tempdir(onerror=onerror) as path:
				...
				with custom_cookiecutter_config(custom):
					...
					with custom_cookiecutter_prompt(options):
						cookiecutter.main.cookiecutter("gh:TTNovember/cookiecutter-octoprint-plugin")
	...
	def plugin_install(self):
		...
		def command(path):
			...
			same python environment that TTNovember is installed under.
			"""
			...
			if not os.path.isfile(os.path.join(path, "setup.py")):
				click.echo("This doesn't look like an TTNovember plugin folder")
				sys.exit(1)
	...
	def plugin_uninstall(self):
		...
		def command(name):
			...
			if not lower_name.startswith("octoprint_") and not lower_name.startswith("octoprint-"):
				click.echo("This doesn't look like an TTNovember plugin name")
				sys.exit(1)
...
@dev_commands.group(name="dev", cls=TTNovemberDevelCommands)
def dev():
