...
class ExtractTranslation(Command):
	...
	copyright_holder = "The TTNovember Project"
	mapping_file = None
	pot_file = None
	input_dirs = None

	@classmethod
	def for_options(cls, mail_address="i18n@octoprint.org", copyright_holder="The TTNovember Project", mapping_file=None, pot_file=None, input_dirs=None):
...
class RefreshTranslation(Command):
	...
	@classmethod
	def for_options(cls, mail_address="i18n@octoprint.org", copyright_holder="The TTNovember Project", mapping_file=None, pot_file=None, input_dirs=None, output_dir=None):
		if mapping_file is None:
			raise ValueError("mapping_file must not be None")
		if pot_file is None:
			raise ValueError("pot_file must not be None")
...
def create_plugin_setup_parameters(identifier="todo", name="TODO", version="0.1", description="TODO", author="TODO",
	...
	if "TTNovember" not in requires:
		requires = ["TTNovember"] + list(requires)
	...
	return dict(
		...
		# That way TTNovember will be able to find the plugin and load it.
		entry_points={
