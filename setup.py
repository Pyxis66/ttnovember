...
EXTRA_REQUIRES = dict(
	# Dependencies for developing TTNovember
	develop=[
	...
	# Dependencies for developing TTNovember plugins
	plugins=[
...
def get_cmdclass():
	...
	# add clean command
	cmdclass.update(dict(clean=octoprint_setuptools.CleanCommand.for_options(source_folder="src", eggs=["TTNovember*.egg-info"])))

	# add translation commands
	translation_dir = "translations"
	pot_file = os.path.join(translation_dir, "messages.pot")
	bundled_dir = os.path.join("src", "octoprint", "translations")
	cmdclass.update(octoprint_setuptools.get_babel_commandclasses(pot_file=pot_file, output_dir=translation_dir, pack_name_prefix="TTNovember-i18n-", pack_path_prefix="", bundled_dir=bundled_dir))
