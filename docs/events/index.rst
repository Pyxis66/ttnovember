...
.. note::
   ...
   With release of TTNovember 1.1.0, the payload data has been harmonized, it is now a key-value-map for all events.
   Additionally, the format of the placeholders in both system command and gcode command triggers has been changed to
   accommodate for this new format. Last but not least, the way of specifying event hooks has changed, TTNovember no longer
...
Event hooks are configured via TTNovember's configuration file ``config.yaml``. There they are contained in a
...
executed on the system TTNovember is running on).
...
.. sourcecode:: yaml
   ...
   events:
     ...
     subscriptions:
     - event: Disconnected
       command: python ~/growl.py -t mygrowlserver -d "Lost connection to printer" -a TTNovember -i http://raspi/Octoprint_logo.png
       type: system
       enabled: false
     - event: PrintStarted
       command: python ~/growl.py -t mygrowlserver -d "Starting {file}" -a TTNovember -i http://raspi/Octoprint_logo.png
       type: system
     - event: PrintDone
       command: python ~/growl.py -t mygrowlserver -d "Completed {file}" -a TTNovember -i http://raspi/Octoprint_logo.png
       type: system
     - event: Connected
       command:
       - M115
...
PrintCancelled
   ...
   Payload:
     ...
     * ``position.t``: last tool selected *through TTNovember* (note that if you did change the printer's selected
       tool outside of TTNovember, e.g. through the printer controller, or if you are printing from SD, this will NOT
       be accurate)
     * ``position.f``: last feedrate for move commands **sent through TTNovember** (note that if you modified the
       feedrate outside of TTNovember, e.g. through the printer controller, or if you are printing from SD, this will
       NOT be accurate)
...
PowerOn
   An ``M80`` was sent to the printer through TTNovember (not triggered when printing from SD!)

PowerOff
   An ``M81`` was sent to the printer through TTNovember (not triggered when printing from SD!)

Home
   A ``G28`` was sent to the printer through TTNovember (not triggered when printing from SD!)

ZChange
   The printer's Z-Height has changed (new layer) through a ``G0`` or ``G1`` that was sent to the printer through TTNovember
   (not triggered when printing from SD!)

Dwell
   A ``G4`` was sent to the printer through TTNovember (not triggered when printing from SD!)

Waiting
   One of the following commands was sent to the printer through TTNovember (not triggered when printing from SD!):
   ``M0``, ``M1``, ``M226``

Cooling
   An ``M245`` was sent to the printer through TTNovember (not triggered when printing from SD!)

Alert
   An ``M300`` was sent to the printer through TTNovember (not triggered when printing from SD!)

Conveyor
   An ``M240`` was sent to the printer through TTNovember (not triggered when printing from SD!)

Eject
   An ``M40`` was sent to the printer through TTNovember (not triggered when printing from SD!)

EStop
   An ``M112`` was sent to the printer through TTNovember (not triggered when printing from SD!)

PositionUpdate
   The response to an ``M114`` was received by TTNovember. The payload contains the current position information
   ...
   Payload:
     ...
     * ``t``: last tool selected *through TTNovember*
     * ``f``: last feedrate for move commands ``G0``, ``G1`` or ``G28`` sent *through TTNovember*
