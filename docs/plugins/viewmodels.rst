...
Register your view model with TTNovember's web app by pushing a config object unto the global array ``OCTOPRINT_VIEWMODELS``.
...
TTNovember will try to inject all view model dependencies requested by your view model. In order to do this it will
...
TTNovember's core currently comes with the following view models that your plugin can request for injection:
...
TTNovember's web application will call several callbacks on all registered view models, provided they implement them.
...
getAdditionalControls()
   Your view model may return additional custom control definitions for inclusion on the "Control" tab of TTNovember's
...
onBeforeWizardTabChange(next, current)
   ...
   completed by the user. Take a look at the "Core Wizard" plugin bundled with TTNovember and the ACL wizard step in
...
onWizardFinish()
   ...
   instruct TTNovember to reload the UI after the wizard closes.
...
.. seealso::
   ...
   `TTNovember's core viewmodels <https://github.com/foosel/TTNovember/tree/devel/src/octoprint/static/js/app/viewmodels>`_
      TTNovember's own view models use the same mechanisms for interacting with each other and the web application as
   ...
   `KnockoutJS documentation <http://knockoutjs.com/documentation/introduction.html>`_
      TTNovember makes heavy use of KnockoutJS for building up its web app.
