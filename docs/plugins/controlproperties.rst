...
As already mentioned earlier, plugins are Python packages which provide certain pieces of metadata to tell TTNovember's
...
``__plugin_settings_overlay__``
  ...
  **default** settings of TTNovember and its plugins that apply when there's no different configuration present in ``config.yaml``. Note that ``config.yaml``
