...
implementations, plugins inform TTNovember about hook handlers using a control property, ``__plugin_hooks__``.
...
TTNovember will call the hook with the define parameters and process the result depending on the hook.

An example for a hook within TTNovember is ``octoprint.comm.protocol.scripts``, which allows adding additional
lines to TTNovember's :ref:`GCODE scripts <sec-features-gcode_scripts>`, either as ``prefix`` (before the existing lines)
...
return a 2-tuple of prefix and postfix if has something for either of those, otherwise ``None``. TTNovember will then take
care to add prefix and suffix as necessary after a small round of preprocessing.

Plugins can easily add their own hooks too. For example, the `Software Update Plugin <https://github.com/TTNovember/TTNovember-SoftwareUpdate>`_
...
.. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_action_command.py
   :linenos:
   :tab-width: 4
   :caption: `custom_action_command.py <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_action_command.py>`_
...
an integer value used for ordering handlers. They way this works is that TTNovember will first sort all registered
...
TTNovember will sort these hooks so that ``orderedhook`` will be called first, then ``anotherorderedhook``, then
...
and start up TTNovember, you'll see output like this:
...
.. note::
   ...
   **always** present in your hook handler declaration.
   ...
   your plugin to stay compatible to TTNovember without any necessary adjustments from you in these cases.
...
.. py:function:: cli_commands_hook(cli_group, pass_octoprint_ctx, *args, **kwargs)
   ...
   By providing a handler for this hook plugins may register commands on TTNovember's command line interface (CLI).
   ...
   The custom ``MultiCommand`` instance :class:`~octoprint.cli.plugins.TTNovemberPluginCommands` is provided
   ...
   **Example:**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_cli_command.py
      :linenos:
      :tab-width: 4
      :caption: `custom_cli_command.py <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_cli_command.py>`_
   ...
   .. note::
      ...
      called without TTNovember initializing your implementation instance. That means that **none** of the
      ...
      *global* settings as ``cli_group.settings``. You can have your handler turn the latter into a
      ...
      TTNovember server context, so there is absolutely no way to access a printer connection, the event bus or
...
.. py:function:: protocol_action_hook(comm_instance, line, action, *args, **kwargs)
   ...
   Hook handlers may use this to react to react to custom firmware messages. TTNovember parses the received action
...
.. py:function:: protocol_gcodephase_hook(comm_instance, phase, cmd, cmd_type, gcode, subcode=None, *args, **kwargs)
   ...
   the printer as well as the ``cmd_type`` parameter used for enqueuing (TTNovember will make sure that the send queue
   will never contain more than one line with the same ``cmd_type``) and the detected gcode command (if it is one).

   Defining a ``cmd_type`` other than None will make sure TTNovember takes care of only having one command of that type
   ...
   **Example**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/rewrite_m107.py
      :linenos:
      :tab-width: 4
      :caption: `rewrite_m107.py <https://github.com/TTNovember/Plugin-Examples/blob/master/rewrite_m107.py>`_
...
.. py:function:: gcode_received_hook(comm_instance, line, *args, **kwargs)
   ...
   **Example:**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/read_m115_response.py
      :linenos:
      :tab-width: 4
      :caption: `read_m115_response.py <https://github.com/TTNovember/Plugin-Examples/blob/master/read_m115_response.py>`_
...
.. py:function:: protocol_scripts_hook(comm_instance, script_type, script_name, *args, **kwargs)
   ...
   **Example:**
   ...
   Appends an ``M117 TTNovember connected`` to the configured ``afterPrinterConnected`` GCODE script.

   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/message_on_connect.py
      :linenos:
      :tab-width: 4
      :caption: `message_on_connect.py <https://github.com/TTNovember/Plugin-Examples/blob/master/message_on_connect.py>`_
...
.. py:function:: serial_factory_hook(comm_instance, port, baudrate, read_timeout, *args, **kwargs)
   ...
   TTNovember, that interface will change in the future. Authors are advised to follow TTNovember's development
...
.. py:function:: file_preprocessor_hook(path, file_object, links=None, printer_profile=None, allow_overwrite=False, *args, **kwargs)
   ...
   **Example**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/strip_all_comments.py
      :linenos:
      :tab-width: 4
      :caption: `strip_all_comments.py <https://github.com/TTNovember/Plugin-Examples/blob/master/strip_all_comments.py>`_
...
.. py:function:: server_bodysize_hook(current_max_body_sizes, *args, **kwargs)
   ...
   The path of the route will be prefixed by TTNovember with ``/plugin/<plugin identifier>/`` (if the path already begins
   with a ``/`` that will be stripped first).

   .. important::

      Implementing this hook will make your plugin require a restart of TTNovember for enabling/disabling it fully.
   ...
   **Example**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/increase_bodysize.py
      :linenos:
      :tab-width: 4
      :caption: `increase_bodysize.py <https://github.com/TTNovember/Plugin-Examples/blob/master/increase_bodysize.py>`_
...
.. py:function:: server_route_hook(server_routes, *args, **kwargs)
   ...
   **Example**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/add_tornado_route.py
      :linenos:
      :tab-width: 4
      :caption: `add_tornado_route.py <https://github.com/TTNovember/Plugin-Examples/blob/master/add_tornado_route.py>`_
...
.. py:function:: templatetype_hook(template_sorting, template_rules, *args, **kwargs)
   ...
   The second entry must be a sorting specification that defines how TTNovember should sort multiple templates injected
   ...
   The third entry must be a rule specification in form of a dict which tells TTNovember how to process the template
   ...
   TTNovember will provide all template configs for custom template types in the Jinja rendering context in the same way
   ...
   **Example**
   ...
   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_template_provider/__init__.py
      :linenos:
      :tab-width: 4
      :caption: `custom_template_provider/__init__.py <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_template_provider/__init__.py>`_

   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_template_provider/templates/custom_template_provider_settings.jinja2
      :linenos:
      :tab-width: 4
      :caption: `custom_template_provider/templates/custom_template_provider_settings.jinja2 <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_template_provider/templates/custom_template_provider_settings.jinja2>`_

   Then the consumer:

   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_template_consumer/__init__.py
      :linenos:
      :tab-width: 4
      :caption: `custom_template_consumer/__init__.py <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_template_consumer/__init__.py>`_

   .. onlineinclude:: https://raw.githubusercontent.com/TTNovember/Plugin-Examples/master/custom_template_consumer/templates/custom_template_consumer_awesometemplate.jinja2
      :linenos:
      :tab-width: 4
      :caption: `custom_template_consumer/templates/custom_template_consumer_awesometemplate.jinja2 <https://github.com/TTNovember/Plugin-Examples/blob/master/custom_template_consumer/templates/custom_template_consumer_awesometemplate.jinja2>`_
