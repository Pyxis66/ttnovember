...
Plugin mixins are the heart of TTNovember's plugin system. They are :ref:`special base classes <sec-plugins-mixins>`
which are to be subclassed and extended to add functionality to TTNovember. Plugins declare their instances that
implement one or multiple mixins using the ``__plugin_implementation__`` control property. TTNovember's plugin core
collects those from the plugins and offers methods to access them based on the mixin type, which get used at multiple
locations within TTNovember.
...
The following snippet taken from TTNovember's code for example shows how all :class:`~octoprint.plugin.AssetPlugin`
...
.. seealso::
   ...
   :ref:`The Plugin Tutorial <sec-plugins-gettingstarted>`
       Tutorial on how to write a simple TTNovember module utilizing mixins for various types of extension.
...
documentation for information on this), TTNovember's plugin subsystem will ensure that the order in which the implementation
...
TTNovember will detect that ``plugin_b`` and ``plugin_c`` define a order number, and since it's identical for both (``1``)
...
:class:`~octoprint.plugin.types.TTNovemberPlugin`,  which also provide attributes of interest to plugin developers.
