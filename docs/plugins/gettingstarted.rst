...
Over the course of this little tutorial we'll build a full fledged, installable TTNovember plugin that displays "Hello World!"
at some locations throughout TTNovember and also offers some other basic functionality to give you an idea of what
you can achieve with TTNovember's plugin system.

First of all let use make sure that you have TTNovember checked out and set up for development on your local
development environment::

  $ cd ~/devel
  $ git clone https://github.com/foosel/TTNovember
  [...]
  $ cd TTNovember
  $ virtualenv venv
  [...]
...
.. important::
   ...
   This tutorial assumes you are running TTNovember 1.3.0 and up. Please make sure your version of
   TTNovember is up to date before proceeding. If you did a fresh checkout, that should already
   be the case but if not you might have to update first. You can check your version of TTNovember
   by running ``octoprint --version`` or by taking a look into the lower left corner in TTNovember's
   web interface.
...
.. code-block:: python
   ...
   __plugin_description__ = "A quick \"Hello World\" example plugin for TTNovember"
...
Saving this as ``helloworld.py`` in ``~/.octoprint/plugins`` yields you something resembling these log entries upon server startup::
   ...
   2015-01-27 11:14:35,124 - octoprint.server - INFO - Starting TTNovember 1.2.0-dev-448-gd96e56e (devel branch)
   [...]
   2015-01-27 11:14:35,124 - octoprint.plugin.core - INFO - Loading plugins from /home/pi/.octoprint/plugins, /home/pi/TTNovember/src/octoprint/plugins and installed plugin packages...
   [...]
...
TTNovember found that plugin in the folder and took a look into it. The name and the version it displays in that log
...
Apart from being discovered by TTNovember, our plugin does nothing yet. We want to change that. Let's make it print
...
and restart TTNovember. You now get this output in the log::

   2015-01-27 11:17:10,792 - octoprint.plugins.helloworld - INFO - Hello World!

Neat, isn't it? We added a custom class that subclasses one of TTNovember's :ref:`plugin mixins <sec-plugins-mixins>`
with :class:`~octoprint.plugin.StartupPlugin` and another control property, ``__plugin_implementation__``, that instantiates
our plugin class and tells TTNovember about it. Taking a look at the documentation of :class:`~octoprint.plugin.StartupPlugin` we see that
this mixin offers two methods that get called by TTNovember during startup of the server, :func:`~octoprint.plugin.StartupPlugin.on_startup` and
...
You'll also note that we are using ``self._logger`` for logging. Where did that one come from? TTNovember's plugin system
...
If you now want to distribute this plugin to other TTNovember users (since it is so awesome to be greeted upon server
...
so that TTNovember will be able to actually find and load them is certainly not impossible, but we want to do it in the
best way possible, meaning we want to make our plugin a fully installable python module that your users will be able to
install directly via `TTNovember's built-in Plugin Manager <https://github.com/foosel/TTNovember/wiki/Plugin:-Plugin-Manager>`_
or alternatively manually utilizing Python's standard package manager ``pip`` directly.

So let's begin. We'll use the `cookiecutter <https://github.com/audreyr/cookiecutter>`_ template for TTNovember plugins here.  This should already be installed if you used the `plugins` extra while installing TTNovember.  However, you may install it with::

   (venv) $ pip install "cookiecutter>=1.4,<1.5"

Then we can use the ``octoprint dev plugin:new`` command [#f1]_ to generate a new TTNovember plugin skeleton for us::
   ...
   plugin_name [TTNovember-Helloworld]:
   repo_name [TTNovember-Helloworld]:
   ...
   plugin_homepage [https://github.com/yourGithubName/TTNovember-Helloworld]:
   plugin_source [https://github.com/yourGithubName/TTNovember-Helloworld]:
   plugin_installurl [https://github.com/yourGithubName/TTNovember-Helloworld/archive/master.zip]:
   (venv) $ cd TTNovember-HelloWorld

.. note::

   If ``octoprint dev plugin:new`` isn't recognized as a command (and also doesn't show up in the output of
   ``octoprint --help``, make sure you installed cookiecutter into the same python environment as TTNovember.

This will create a project structure in the ``TTNovember-HelloWorld`` folder we just changed to that looks like this::
...
.. code-block:: python
   ...
   plugin_description = """A quick "Hello World" example plugin for TTNovember"""
   plugin_author = "Your Name"
   plugin_author_email = "you@somewhere.net"
   plugin_url = "https://github.com/yourGithubName/TTNovember-Helloworld"
   plugin_license = "AGPLv3"
...
discoverable by TTNovember, however we don't have to reinstall it after any changes we will still do. We can have the
``octoprint dev plugin:install`` command do everything for us here, it will ensure to use the python binary belonging
to your TTNovember installation::

   (venv) $ octoprint dev plugin:install
   running develop
   running egg_info
   creating TTNovember_HelloWorld.egg-info
   [...]
   Finished processing dependencies for TTNovember-HelloWorld==1.0.0

Restart TTNovember. Your plugin should still be properly discovered and the log line should be printed::

   2015-01-27 13:43:34,134 - octoprint.server - INFO - Starting TTNovember 1.2.0-dev-448-gd96e56e (devel branch)
   [...]
   2015-01-27 13:43:34,134 - octoprint.plugin.core - INFO - Loading plugins from /home/pi/.octoprint/plugins, /home/pi/TTNovember/src/octoprint/plugins and installed plugin packages...
   [...]
   2015-01-27 13:43:34,818 - octoprint.plugin.core - INFO - 3 plugin(s) registered with the system:
   [...]
   | Hello World (1.0.0) = /home/pi/devel/TTNovember-HelloWorld/octoprint_helloworld
   [...]
...
The nice thing about our plugin now being a proper python package is that TTNovember can and will access the metadata defined
...
and restart TTNovember::

   2015-01-27 13:46:33,786 - octoprint.plugin.core - INFO - 3 plugin(s) registered with the system:
   [...]
   | TTNovember-HelloWorld (1.0.0) = /home/pi/devel/TTNovember-HelloWorld/octoprint_helloworld
   [...]

Our "Hello World" Plugin still gets detected fine, but it's now listed under the same name it's installed under,
"TTNovember-HelloWorld". That's a bit redundant and squashed, so we'll override that bit via ``__plugin_name__`` again:
...
Restart TTNovember again::

   2015-01-27 13:48:54,122 - octoprint.plugin.core - INFO - 3 plugin(s) registered with the system:
   [...]
   | Hello World (1.0.0) = /home/pi/TTNovember-HelloWorld/octoprint_helloworld
   [...]
...
Following the README of the `Plugin Skeleton <https://github.com/TTNovember/TTNovember-PluginSkeleton>`_ you could now
already publish your plugin on Github and it would be directly installable by others using pip::

   (venv) $ pip install https://github.com/yourGithubName/TTNovember-HelloWorld/archive/master.zip

But let's add some more features instead.

.. _sec-plugins-gettingstarted-templates:

Frontend fun: How to add functionality to TTNovember's web interface
-------------------------------------------------------------------

Outputting a log line upon server startup is all nice and well, but we want to greet not only the administrator of
our TTNovember instance but actually everyone that opens TTNovember in their browser. Therefore, we need to modify
TTNovember's web interface itself.

We can do this using the :class:`TemplatePlugin` mixin. For now, let's start with a little "Hello World!" in TTNovember's
...
Restart TTNovember and open the web interface in your browser (make sure to clear your browser's cache!).
...
which is a little settings manager TTNovember conveniently injects into our Plugin when we include the :class:`~octoprint.plugin.SettingsPlugin`
mixin.
...
Restart TTNovember. You should see something like this::
...
TTNovember injects the template variables that your plugin defines prefixed with ``plugin_<plugin identifier>_`` into
...
Restart TTNovember and shift-reload the page in your browser (to make sure you really get a fresh copy). The link should
still work and point to the URL we defined as default.

Let's change the URL! Open up your TTNovember instance's ``config.yaml`` file and add the following to it (if a ``plugins``
...
Restart TTNovember. Not only should the URL displayed in the log file have changed, but also the link should now (after
...
Nice! But not very user friendly. We don't have any way yet to edit the URL from within TTNovember and have to restart
...
TTNovember's settings. Accessing plugin settings hence works by following the path under which they are stored in
TTNovember's internal settings data model (made public via the ``config.yaml``), ``plugins.<plugin identifier>.<configuration key>``.
...
If you were now to restart TTNovember and reload the web interface, you'll get the settings dialog placed just fine
in TTNovember's settings, and the link would also still show up in the navigation bar, but both the input field of the
settings dialog as well as the link's ``href`` attribute would not show our link. The reason for this is that TTNovember
by default assumes that you'll want to bind your own view models to your templates and hence "unbinds" the included
templates from the templates that are in place at the injected location already. In order to tell TTNovember to please
...
:func:`~octoprint.plugin.TemplatePlugin.get_template_configs` method. We'll tell TTNovember to use no custom bindings
...
Restart TTNovember and shift-reload your browser. Your link in the navigation bar should still point to the URL we
...
.. note::
   ...
   The way we've done our data binding and how TTNovember currently works, your link's target will update immediately
   when you update the value in the settings dialog. Even if you click Cancel instead of Save, the change will still
   be reflected in the UI but will be overwritten again by the stored data upon a reload. This is caused by TTNovember
...
In the previous section we set that ``custom_bindings`` parameter to ``False`` since we wanted TTNovember to bind the
...
add an additional UI component to our TTNovember interface, a custom tab. It will act as a little internal web browser,
...
We need to tell TTNovember about this new static asset so that it will properly inject it into the page. For this we
...
Remember how we only added those since we wanted TTNovember to use existing bindings on our navigation bar and settings
menu entries? We don't want this this time, and we named our tab template such that TTNovember will pick it up automatically
...
.. code-block:: javascript
   ...
   $(function() {
       function HelloWorldViewModel(parameters) {
           ...
           // gets called _after_ the settings have been retrieved from the TTNovember backend and thus
...
Take a close look at lines 31 to 42. This is how our plugin tells TTNovember about our new view model, how to
instantiate it, which dependencies to inject and to which elements in the final page to bind. Since we want to access
the URL from the settings of our plugin, we'll have TTNovember inject the ``SettingsViewModel`` into our own view model,
which is registered within TTNovember under the name ``settingsViewModel``. We'll only bind to our custom tab
for now, which TTNovember will make available in a container with the id ``tab_plugin_helloworld`` (unless otherwise
configured).
...
Restart TTNovember and shift-reload the browser. You should see a shiny new "Hello World" tab right at the end of the
tab bar. Click on it!
...
TTNovember by default bundles all CSS, JavaScript and LESS files to reduce the amount of requests necessary to fully
load the page. But in order to fully be able to see how what we just did changes how our plugin interacts with TTNovember
we want to disable that behaviour for now. Open up TTNovember's ``config.yaml`` and disable bundling of the webassets:
...
Restart TTNovember, shift-reload your browser and take a look. Everything should still look like before, but now
TTNovember included our stylesheet and the style information for the ``iframe`` is taken from that instead of
...
something like LESS for generating your CSS from. If you use `LESS <http://lesscss.org/>`_, which is what TTNovember
uses for that purpose, you can even put TTNovember into a mode where it directly uses your LESS files instead of the
...
and enable LESS mode by adjusting one of TTNovember's ``devel`` flags via the ``config.yaml`` file:
...
Restart TTNovember and shift-reload. Your "Hello World" tab should still look like before. Take a look at the site's
source code. In the ``head`` section of the page you'll see that instead of your ``helloworld.css`` TTNovember now
...
Remember when I mentioned that TTNovember by default bundles all our assets for us? We adjusted our ``config.yaml`` to
...
.. note::
   ...
   If your plugin only provides CSS files, TTNovember will detect this when switched to LESS mode and include your
   ...
   The same thing works the other way around too by the way. If your plugin only provides LESS files, TTNovember will link to
...
You've now seen how easy it is to add functionality to TTNovember with this little tutorial. You can find the full
source code of the little Hello World plugin we built together here `on Github <https://github.com/TTNovember/Plugin-Examples/tree/master/helloworld>`_.

But I want to invite you to dive deeper into TTNovember's plugin system. To get an idea of all the other various plugin types
you haven't seen yet, :ref:`take a look at the available plugin mixins <sec-plugins-mixins>`.

For some insight on how to create plugins that react to various events within TTNovember,
`the Growl Plugin <https://github.com/TTNovember/TTNovember-Growl>`_ might be a good example to learn from. For how to
add support for a slicer, TTNovember's own bundled `CuraEngine plugin <https://github.com/foosel/TTNovember/wiki/Plugin:-Cura>`_
might give some hints. For extending TTNovember's interface, the `NavbarTemp plugin <https://github.com/imrahil/TTNovember-NavbarTemp>`_
...
.. seealso::
   ...
   `Jinja Template Designer Documentation <http://jinja.octoprint.org/templates.html>`_
      ...
      by TTNovember's frontend. Linked here are the docs for Jinja 2.8.1, which TTNovember still
...
.. [#f1] Instead of the ``octoprint dev plugin:new`` you could also have manually called cookiecutter with the
         template's repository URL shortcut: ``cookiecutter gh:TTNovember/cookiecutter-octoprint-plugin``. The
...
.. [#f3] Please always consult the Jinja documentation at `jinja.octoprint.org <http://jinja.octoprint.org>`_ instead of
         ...
         compatibility reasons TTNovember currently sadly has to rely on an older version of Jinja. The documentation
