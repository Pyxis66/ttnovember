...
.. http:get:: /api/files/(string:location)
   ...
   **Example**:
   ...
   :param location: The origin location from which to retrieve the files. Currently only ``local`` and ``sdcard`` are
                    supported, with ``local`` referring to files stored in TTNovember's ``uploads`` folder and ``sdcard``
...
.. http:post:: /api/files/(string:location)
   ...
   **Example for creating a folder**
   ...
   :param location:  The target location to which to upload the file. Currently only ``local`` and ``sdcard`` are supported
                     here, with ``local`` referring to TTNovember's ``uploads`` folder and ``sdcard`` referring to
...
.. http:post:: /api/files/(string:location)/(path:path)
   ...
   move
     ...
     * ``destination``: The path of the parent folder to which to move the file or folder.
     ...
     in use by TTNovember (e.g. it is a GCODE file that's currently being printed) a :http:statuscode:`409` will be
     returned.
   ...
   **Example Move Request**
   ...
   :param location:             The target location on which to send the command for is located, either ``local`` (for TTNovember's ``uploads``
...
.. http:delete:: /api/files/(string:location)/(path:path)
   ...
   **Example Request**
   ...
   :param location: The target location on which to delete the file, either ``local`` (for TTNovember's ``uploads``
                    folder) or ``sdcard`` for the printer's SD card (if available)
...
.. list-table::
   ...
   * - ``free``
     - 0..1
     - String
     - The amount of disk space in bytes available in the local disk space (refers to TTNovember's ``uploads`` folder). Only
