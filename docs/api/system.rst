   ...
   .. sourcecode:: http
...

      {
        "core": [
          ...
          {
            "action": "restart",
            "name": "Restart TTNovember",
            "command": "sudo service octoprint restart",
            "confirm": "You are about to restart the TTNovember server.",
            "async": true,
            "ignore": true,
            "source": "core",
...
.. http:post:: /api/system/commands/(string:source)/(string:action)
   ...
   **Example**
   ...
   Restart TTNovember via the core system command ``restart`` (which is available if the server
...
.. list-table::
   ...
   * - ``source``
     ...
     - Source of the command definition, currently either ``core`` (for system actions defined by
       TTNovember itself) or ``custom`` (for custom system commands defined by the user through ``config.yaml``).
   * - ``resource``
     - 1
     - string
