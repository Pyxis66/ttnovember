...
printer. TTNovember currently knows the following components:
...
Besides that, TTNovember also provides a :ref:`full state report of the printer <sec-api-printer-state>`.
...
.. note::
   ...
   TTNovember's internal webserver is single threaded and can only handle one request at a time. This is
   ...
   Hence TTNovember currently doesn't offer any synchronous way of retrieving the output of responses from the printer.
...
.. http:post:: /api/printer/sd
   ...
   init
     ...
     .. note::
        If TTNovember detects the availability of a SD card on the printer during connection, it will automatically attempt
        to initialize it.
...
.. http:get:: /api/printer/sd
   ...
   If SD support has been disabled in TTNovember's settings, a :http:statuscode:`404` is returned.
   ...
   **Example**
   ...
   :statuscode 200: No error
   :statuscode 404: If SD support has been disabled in TTNovember's config.
