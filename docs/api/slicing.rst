...
The Slicing API on one hand offers methods for managing slicing profiles stored within TTNovember, on the other hand
...
.. list-table::
   ...
   * - ``sameDevice``
     - 1
     - ``boolean``
     - Whether the slicer runs on the same device as TTNovember (``true``) and hence can't be used while printing,
       or not (``false``)
   * - ``default``
     - 1
     - ``boolean``
