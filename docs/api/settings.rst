...
.. http:get:: /api/settings
   ...
   Retrieves the current configuration of TTNovember.
...
.. http:post:: /api/settings
   ...
   Saves the provided settings in TTNovember.
...
.. list-table::
   ...
   * - ``api.key``
     - Only maps to ``api.key`` in ``config.yaml`` if request is sent with admin rights, set to ``n/a`` otherwise.
       Starting with TTNovember 1.3.3 setting this field via :ref:`the API <sec-api-settings-save>` is not possible,
