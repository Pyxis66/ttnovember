...
of TTNovember or installed plugins which registered with it and -- if properly
...
updates that might be available for your TTNovember installation or any plugins
that registered themselves with it. In order to also be able to update
your TTNovember installation, you'll need to configure
at least TTNovember's checkout folder, and you also should
configure the restart commands for TTNovember and the whole server.

For configuring the plugin you'll need to go into TTNovember's Settings Dialog, navigate to the
...
There you can adjust the following settings:
  ...
  * **TTNovember checkout folder**: This should be the path to TTNovember's git checkout folder (``/home/pi/TTNovember``
    for OctoPi or `manual installs following the Raspberry Pi setup guide <https://github.com/foosel/TTNovember/wiki/Setup-on-a-Raspberry-Pi-running-Raspbian>`_).
    This must be set to allow updating from within TTNovember
    ...
    .. note::
       ...
       **OctoPi 0.11.0 users**: Please also take a look at
       `the note at the very end of this FAQ entry <https://github.com/foosel/TTNovember/wiki/FAQ#how-can-i-update-the-octoprint-installation-on-my-octopi-image>`_.
       Due to a little issue in that OctoPi release 0.11.0 you might have to fix
       the URL your TTNovember checkout is using for updating. This can easily be
       done by SSHing into your OctoPi instance and doing this::

           cd ~/TTNovember
           git remote set-url origin https://github.com/foosel/TTNovember.git

  * **TTNovember version tracking**: Whether you want to track TTNovember *releases* or every *commit*. Usually you want to
    select "Release" here which is also the default, unless you are a developer.
  * **TTNovember Release Channel**: The release channel of TTNovember to track for updates. If you only want stable versions,
    ...
    "Stable". `Read more about Release Channels here <https://github.com/foosel/TTNovember/wiki/Using-Release-Channels>`_.
  * **Version cache TTL**: The "time to live" of the cache TTNovember will use to temporarily persist the version information
...
That restart commands for TTNovember and the whole server can be configured under Settings > Server.
...
The functionality of the Software Update Plugin is also available on TTNovember's command line interface under the
...
.. code-block:: none
   ...
   Usage: octoprint plugins softwareupdate:check [OPTIONS] [TARGETS]...
     ...
     - octoprint plugins softwareupdate:check octoprint
         This will only check TTNovember itself for available
         updates.
...
.. code-block:: none
   ...
   Usage: octoprint plugins softwareupdate:update [OPTIONS] [TARGETS]...
     ...
     - octoprint plugins softwareupdate:update octoprint
         This will only update TTNovember and leave any further
...
.. code-block:: yaml
    ...
    plugins:
      softwareupdate:
        ...
        checks:
          # "octoprint" is reserved for TTNovember
          octoprint:
            # this defines an version check that will check against releases
            # published on TTNovember's Github repository and an update method
            # utilizing an (included) update script that will be run on
            # TTNovember's checkout folder
            type: github_release
            user: foosel
            repo: TTNovember
...
++++++++++++++
  ...
  * ``update_script``: A script to execute in order to perform the update. May
    ...
    of the script and ``{python}`` for the python executable TTNovember is
...
Example for a setup that allows "bleeding edge" updates of TTNovember under
     ...
     softwareupdate:
       checks:
         octoprint:
           ...
           repo: TTNovember
           branch: devel
           update_folder: /home/pi/TTNovember

Plugin installed via pip and hosted on Github under
``https://github.com/someUser/TTNovember-SomePlugin``, only releases should be
tracked:
     ...
     softwareupdate:
       checks:
         some_plugin:
           type: github_release
           user: someUser
           repo: TTNovember-SomePlugin
           pip: 'https://github.com/someUser/TTNovember-SomePlugin/archive/{target}.zip'
...
.. py:function:: update_config_hook(*args, **kwargs)
   ...
   **Example**
   ...
   at the (fictional) repository ``https://github.com/someUser/TTNovember-UpdatePluginDemo``.
      ...
      def get_update_information(*args, **kwargs):
          return dict(
              updateplugindemo=dict(
                  ...
                  user="someUser",
                  repo="TTNovember-UpdatePluginDemo",

                  pip="https://github.com/someUser/TTNovember-UpdatePluginDemo/archive/{target}.zip"
              )
          )
...
The source of the Software Update plugin is bundled with TTNovember and can be
