...
The JS Client Library provides an interface to all of TTNovember's API, including the SockJS based socket to send
...
TTNovember instance's base URL (e.g. ``http://octopi.local/static/webassets/packed_client.js``).

If you are using it from a web page hosted on TTNovember as a Jinja2 template, you should use one of the following
methods to embed it instead of manually entering the URL, in order to have TTNovember take care of setting the
correct URL prefix:
...
When you import the client library as described above, a global variable ``TTNovember`` will become available, which is
a prepared instance of the ``TTNovemberClient`` class the library assembles from registered components. You can directly
used that singular ``TTNovember`` instance if you only need to talk to one TTNovember server:

.. code-block:: javascript

   TTNovember.options.baseurl = "http://example.com/octoprint/"
   TTNovember.options.apikey = "apikey";

   TTNovember.files.list()
...
.. code-block:: javascript
   ...
   var client1 = new TTNovemberClient({baseurl: "http://example.com/octoprint1/", apikey: "apikey1"});

   var client2 = new TTNovemberClient();
...
.. seealso::
   ...
   `TTNovember-ForceLogin <https://github.com/TTNovember/TTNovember-ForceLogin>`_
       A plugin that disables anonymous access to the regular TTNovember UI by implementing a custom UI. Utilizes the
