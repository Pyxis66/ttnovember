...
TTNovember allows you to add custom controls to the "Control" tab of its interface. Control types reach from simple
...
Adding this to ``config.yaml``, restarting the TTNovember server and switching to the "Control" tab within its
...
.. hint::
   ...
   which allows you configuring your Custom Controls through TTNovember's
...
.. list-table::
   ...
   * - ``input``
     - (Optional) A list of definitions of input parameters for a ``command`` or ``commands``, to be rendered as
       ...
       In TTNovember's default UI input fields are always rendered left to right (inline).
   * - ``input.name``
