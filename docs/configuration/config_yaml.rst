...
If not specified via the command line, the main configuration file ``config.yaml`` for TTNovember is expected in its
settings folder, which unless defined differently via the command line is located at ``~/.octoprint`` on Linux, at
``%APPDATA%/TTNovember`` on Windows and at ``~/Library/Application Support/TTNovember`` on MacOS. If the file is not there,
you can just create it - it will only get created by TTNovember once you save settings that deviate from the default
settings.

Note that many of these settings are available from the "Settings" menu in TTNovember itself.
...
.. code-block:: yaml
   ...
   accessControl:
     ...
     # take place. If available TTNovember will evaluate the "X-Forwarded-For" HTTP header for determining
...
Use the following settings to tweak TTNovember's appearance a bit to better distinguish multiple instances/printers
...
.. code-block:: yaml
   ...
   appearance:
     ...
     # (as "<Name> [TTNovember]") and in the navigation bar (as "TTNovember: <Name>")
     name: My Printer
     ...
     components:
       ...
       # be included in TTNovember's UI at all. Note that this might mean that critical
     ...
     # Default language of TTNovember. If left unset TTNovember will try to match up available
     # languages with the user's browser settings.
     defaultLanguage: null

.. note::

   By modifying the ``components`` > ``order`` lists you may reorder TTNovember's UI components as you like. You can also
   ...
   in TTNovember, you'd need to redefine ``components`` > ``order`` > ``tab`` by including something like this in your
   ``config.yaml``:
   ...
   TTNovember will then turn this into the order ``plugin_helloworld``, ``temperature``, ``control``, ``gcodeviewer``,
...
The following settings are only relevant to you if you want to do TTNovember development:

.. code-block:: yaml

   # Settings only relevant for development
   devel:
     # Settings for TTNovember's internal caching
     cache:
     ...
     # Settings for stylesheet preference. TTNovember will prefer to use the stylesheet type
     # specified here. Usually (on a production install) that will be the compiled css (default).
     # Developers may specify less here too.
     stylesheet: css

     # Settings for TTNovember's web asset merging and minifying
     webassets:
       # If set to true, TTNovember will merge all JS, all CSS and all Less files into one file per type
       ...
       # If set to true, TTNovember will minify its viewmodels (that includes those of plugins). Note: if bundle is
     ...
     virtualPrinter:
       ...
       # Size of the simulated RX buffer in bytes, when it's full a send from TTNovember's
       # side will block
       rxBuffer: 64
...
The following settings provide parameters for estimators within TTNovember. Currently only
...
.. note::
   ...
   .. code-block:: yaml
      ...
      events:
        subscriptions:
        - event: Startup
          command: "logger 'TTNovember started up'"
          type: system
          debug: true

   This will be logged in TTNovember's logfile as

   .. code-block:: none

      Executing System Command: logger 'TTNovember started up'
...
Use the following settings to enable or disable TTNovember features:
...
.. code-block:: yaml
   ...
   feature:
     ...
     # Specifies whether TTNovember should wait for the start response from the printer before trying to send commands
     # during connect.
     waitForStartOnConnect: false

     # Specifies whether TTNovember should send linenumber + checksum with every printer command. Needed for
     # successful communication with Repetier firmware
     alwaysSendChecksum: false

     # Specifies whether TTNovember should also send linenumber + checksum with commands that are *not*
     # detected as valid GCODE (as in, they do not match the regular expression "^\s*([GM]\d+|T)").
     sendChecksumWithUnknownCommands: false

     # Specifies whether TTNovember should also use up acknowledgments ("ok") for commands that are *not*
     # detected as valid GCODE (as in, they do not match the regular expression "^\s*([GM]\d+|T)").
...
Use the following settings to set custom paths for folders used by TTNovember:

.. code-block:: yaml

   folder:
     # Absolute path where to store gcode uploads. Defaults to the uploads folder in the TTNovember settings folder
     uploads: /path/to/upload/folder

     # Absolute path where to store finished timelapse recordings. Defaults to the timelapse folder in the TTNovember
     # settings dir
     timelapse: /path/to/timelapse/folder

     # Absolute path where to store temporary timelapse files. Defaults to the timelapse/tmp folder in the TTNovember
     # settings dir
     timelapse_tmp: /path/to/timelapse/tmp/folder

     # Absolute path where to store log files. Defaults to the logs folder in the TTNovember settings dir
     ...
     # added to TTNovember (and deleted from that folder). Can e.g. be used to define a folder which
...
.. code-block:: yaml
   ...
   scripts:
     ...
     gcode:
       ...
       # Script called after TTNovember connected to the printer.
...
.. code-block:: yaml
   ...
   serial:
     ...
     # dead and TTNovember disconnects with an error.
     ...
     # with the printer is considered dead and TTNovember will disconnect with an error
     maxWritePasses:
...
.. code-block:: yaml
   ...
   server:
     ...
     # If this option is true, TTNovember will show the First Run wizard and set the setting to
     # false after that completes
     firstRun: false

     # If this option is true, TTNovember will enable safe mode on the next server start and
     ...
     # Settings if TTNovember is running behind a reverse proxy (haproxy, nginx, apache, ...).
     # These are necessary in order to make TTNovember generate correct external URLs so
     # that AJAX requests and download URLs work.
     reverseProxy:

       # The request header from which to determine the URL prefix under which TTNovember
       # is served by the reverse proxy
       prefixHeader: X-Script-Name

       # The request header from which to determine the scheme (http or https) under which
       # a specific request to TTNovember was made to the reverse proxy
       schemeHeader: X-Scheme

       # The request header from which to determine the host under which TTNovember
       # is served by the reverse proxy
       hostHeader: X-Forwarded-Host

       # Use this option to define an optional URL prefix (with a leading /, so absolute to your
       # server's root) under which to run TTNovember. This should only be needed if you want to run
       # TTNovember behind a reverse proxy under a different root endpoint than `/` and can't configure
       ...
       # TTNovember. This should only be needed if you want to run TTNovember behind a reverse
       ...
       # Use this option to define an optional forced host under which to run TTNovember. This should
       # only be needed if you want to run TTNovember behind a reverse proxy with a different hostname
       # than TTNovember itself but can't configure said reverse proxy to send a host HTTP header
       # (X-Forwarded-Host by default, see above) with forwarded requests.
       hostFallback:

     # Settings for file uploads to TTNovember, such as maximum allowed file size and
     # header suffixes to use for streaming uploads. TTNovember does some nifty things internally in
     ...
     commands:
       ...
       # Command to restart TTNovember, defaults to being unset
       serverRestartCommand: sudo service octoprint restart

       # Command to restart the system TTNovember is running on, defaults to being unset
       systemRestartCommand: sudo shutdown -r now

       # Command to shut down the system TTNovember is running on, defaults to being unset
...
.. note::
   ...
   If you want to run TTNovember behind a reverse proxy such as HAProxy or Nginx and use a different base URL than the
   server root ``/`` you have two options to achieve this. One approach is using the configuration settings ``baseUrl`` and
   ``scheme`` mentioned above in which TTNovember will only work under the configured base URL.
   ...
   If you use these headers TTNovember will work both via the reverse proxy as well as when called directly. Take a look
   `into TTNovember's wiki <https://github.com/foosel/TTNovember/wiki/Reverse-proxy-configuration-examples>`_ for some
...
Use the following settings to add custom system commands to the "System" dropdown within TTNovember's top bar.
...
TTNovember is running is allowed to do this without password entry:
...
.. code-block:: yaml
   ...
   webcam:
     ...
     # Whether to include a "created with TTNovember" watermark in the generated timelapse movies
     watermark: true
